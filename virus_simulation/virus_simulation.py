...
class Patient(object):
    ...
    def update(self):
        ...
        for v in self.viruses:
            ...
            try:
                baby = v.reproduce(popDensity)
                if type(baby) is SimpleVirus:
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for d in activeDrugs:
            if self.isResistantTo(d) == False:
                raise NoChildException()
        if random.random() < (self.getMaxBirthProb()*(1-popDensity)):
            childResistance = self.resistances
            for r in childResistance:
                if random.random() < self.getMutProb():
                    if childResistance[r] == True:
                        childResistance[r] = False
                    else:
                        childResistance[r] = True
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        for v in self.viruses:
            check = False
            resistances = v.getResistances().copy()
            for d in drugResist:
                if d in resistances and resistances[d] == False:
                    check = True
                elif d not in resistances:
                    check = True
            if check == False:
                resistPop+=1
        return resistPop
    ...
    def update(self):
        ...
        for v in self.viruses:
            if v.doesClear() == True:
                self.viruses.remove(v)
        parents = self.viruses[:]
        for v in parents:
            try:
                popDensity = (float(len(self.viruses))/float(self.maxPop))
                child = v.reproduce(popDensity,activeDrugs)
                if type(child) is ResistantVirus:
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for t in range(numTrials):
        ...
        pat = TreatedPatient(viri,maxPop)
        for s in range(150):
            pat.update()
            tot = (float(pat.getTotalPop())/float(numTrials))
            res = (float(pat.getResistPop(['guttagonol']))/float(numTrials))
            total[s]+=tot
            resistant[s]+=res
        pat.addPrescription('guttagonol')
        for s in range(150):
            pat.update()
            tots = (float(pat.getTotalPop())/float(numTrials))
            ress = (float(pat.getResistPop(['guttagonol']))/float(numTrials))
            total[s+150]+=tots
            resistant[s+150]+=ress
        for e in range(300):
            total[e] = round(total[e],5)
            resistant[e] = round(resistant[e],5)
    #pylab.plot(total)
    #pylab.plot(resistant)
